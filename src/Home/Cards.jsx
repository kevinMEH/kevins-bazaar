...
const Products = () => {
    return (
        <div>
            <h2 className="text-darkGray font-bold text-3xl md:text-4xl lg:text-5xl text-center px-4 pb-5 md:pb-7 lg:pb-9">Check out our newest releases.</h2>
...
const Card = ({ image, alt, end = false }) => {
    return (
        <Link to={"catalog"} className={ end ? "relative block rounded-xl overflow-hidden" : "block rounded-xl overflow-hidden" }>
            <img className="object-cover h-52 lg:h-64 w-full" src={image} alt={alt} />
            { end ? <>
